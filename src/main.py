...
def run_sync_browser_extraction(config: InputModel, run_id: str, logger):
    """
    Run the Playwright sync browser extraction in a separate thread.
    This allows sync Playwright code to run outside the async event loop.
    
    Returns tuple: (actions, mcp_json, screenshot_data, preview_html, url)
    """
